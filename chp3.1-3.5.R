...
# Otherwise the colors of areas will no longer be distinct and it will be hard to visually pick
# out the patterns of different categories. Transparency (alpha) does not work well with colors 
...
# As noted before, as the number of categories increases, the difference between colors decreases, 
# to the point that the color of categories will no longer be visually distinct.

# VI. Read ?facet_wrap. What does nrow do? What does ncol do? What other options control the layout 
...
# It is necessary since facet_wrap() only facets on one variable. These arguments are unnecessary for 
# facet_grid() since the number of rows and columns are determined by the number of unique values of the 
# variables specified.
...
# It is easier to compare relative levels of y by scanning horizontally, 
# so it may be easier to visually compare these levels.